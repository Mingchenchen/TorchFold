...
class CrossAttention(nn.Module):
    def __init__(self, key_dim: int = 128, value_dim: int = 128, c_single_cond: int = 128, num_head: int = 4) -> None:
        ...
        # Match training: use register_buffer so q_scale is in state_dict, avoiding
        # unexpected keys when loading checkpoints.
    ...
    def forward(
        ...
        # bias = (
        #     torch.Tensor([1e9]).to(torch.bfloat16).npu()
        #     * mask_q.logical_not().unsqueeze(-2).unsqueeze(-1)
        #     * mask_k.logical_not().unsqueeze(-2).unsqueeze(-2)
        # )
        bias = (
            torch.Tensor([1e9]).to(torch.float32).cuda()
            * mask_q.logical_not().unsqueeze(-2).unsqueeze(-1)
            * mask_k.logical_not().unsqueeze(-2).unsqueeze(-2)
        )
